...
;;                                * works only from DEI, Padova.
;; Credits : Stan Lanning for the defadvice code that prevents indenting
...
;; Created: 01 Feb 1994
;; $Id: html-helper-mode.el,v 3.0.0.0 1998/08/06 18:53:03 nelson Exp $
...
;; Changes from G.U. Lauri

;; Revised font lock regexps (improvements welcome) to higlight phtml and asp
;; scripts (JSP script support should be easy to implement)

;; Added new character entities (letters with grave or acute accents common in
;; italian language)

;; Added function to narrow the editing to Server Scripts, changing mode 
;; accordingly

;; Added code to prevent indentation of <PRE></PRE> sections (Stan Lanning)
;; Added code to preserve cursor location (Stan Lanning)
;; Corrected the use of faces, now standard faces are not changed when you
;; change html-helper-mode ones. Removed references to Bold-Italic fonts.
;; Added smart code to dynamically figure out which narrowing function use 
;; and kept original single key binding (Stan Lanning)
...
(defconst html-helper-mode-version (substring "$Revision: 3.0.0.0 $" 11 15))
...
;; Once html-helper-m
;; Oncede is aware of a type, it can then install the
...
(mapcar
 ...
 '(
   ...
   (form    "m"	      "<input"		"Submit Form"   	  ("<input type=\"SUBMIT\" value=\"" (p "Submit button text: ") "\">"))
   (form    "b"       "<input"          "Button"                  ("<input type=\"BUTTON\" value=\"" (p "Button Text: ") "\" name=\"" (p "Button Name: ") "\">"))
   ...
   ;; table 
   (table   "t"       "<table>"         "Table"           ("<TABLE BORDER WIDTH="(p " Width " )" >\n</TABLE>"))
   (table   "r"       "<tr>"         "Table Row"           ("<TR>\n</TR>"))
   (table   "d"       "<td>"         "Table Data"           ("<TD align="(p " Align " )" colspan="(r " Colspan ")" ></TD>"))
   ...
   ;; scripting elements
   (script  "j"    "<SCRIPT>"       "JavaScript"        ("<SCRIPT TYPE=\"text/javascript\">\n</SCRIPT>"))
   (script  "v"    "<SCRIPT>"       "VBScript"        ("<SCRIPT TYPE=\"text/vbscript\">\n</SCRIPT>"))
   (script  "%"    "<%="            "ASP output"      ("<%="(p " Variabile: ")"%>"))
   (script  "a"    "<%xx%>"         "ASP code"        ("<%\n"(r " Code: " )"\n%>"))
   (script  "<"    "<%xx%>"         "ASP break"        ("%>\n"(r " Code: " )"\n<%"))
   (script  "="    "<?="            "PHP output"      ("<?="(p " Variabile: ")"?>"))
   (script  "p"    "<?xx?>"         "PHP code"      ("<? PHP\n"(r " Code: " )"\n?>"))
   (script  "?"    "<?xx?>"         "PHP break"        ("?>\n"(r " Code: " )"\n<? PHP"))
...
For more documentation and the original mode, 
...
(defun asp-html-helper-mode ()
  "wraparound of basic mode, to customize the template"
  (interactive)
  (base-html-helper-mode "ASP"))

(defun html-helper-mode ()
  "wraparound of basic mode, to customize the template for HTML"
  (interactive)
  (base-html-helper-mode "HTML"))

(defun php-html-helper-mode ()
  "wraparound of basic mode, to customize the template for PHP"
  (interactive)
...
(if (string-match "XEmacs\\|Lucid" (emacs-version))
  ...
  (if (string-lessp "20.4.1" emacs-version)
      (progn
	...
	(let ((internal-background-mode 
	       (or (if (boundp 'font-lock-background-mode) font-lock-background-mode nil)
              (if (boundp 'frame-background-mode) frame-background-mode nil)
...
(defvar html-helper-font-lock-keywords
  (let (;; Titles and H1's, like function defs.
    ...
    (list ; Avoid use of `keep', since XEmacs will treat it the same as `t'.
     ...
     ;; Paint ASP skripts in font-lock-builtin-face, regexp by Larry
     ;; Smith <lsmith@cio2000.eds.com>: '("<%[\000-\177]*%>" 1
     ;; font-lock-builtin-face). Regexp by Tony Graham
     ;; <tkg@menteith.com> character coding independent:
     ;; '("\\(<%\\([^%]\\|%[^>]\\|\\)*%>\\)" Final regexp, now a
     ;; character string can contain the ASP script end marker, i.e.
     ;; " foobar %> "
     ;;
     '("\\(<%\\([^%]\\|%[^>]\\|\\(\"\\(%>\\|.\\)*\"\\)\\)*%>\\)" 
...
;;{{{ mode provisions & hook execution
